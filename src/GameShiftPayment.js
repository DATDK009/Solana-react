...
const GameShiftPayment = ({ onSale }) => {
  ...
  const handleBuy = async () => {
    ...
    try {
      const response = await fetch(`https://api.gameshift.dev/nx/unique-assets/${item.item.id}/buy`, {
        method: 'POST',
        body: JSON.stringify(purchaseData),
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
          'x-api-key': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJrZXkiOiI4ZGI1MTRiMC01YzJlLTRlMGItYmQ2Mi0zNjBiOThhZDZjZGYiLCJzdWIiOiJhZThjOTk0OS04MjUyLTQwNmUtODBkMS1iMzhhNDY4MWE4YzIiLCJpYXQiOjE3MzI0MzgwNzl9.JLD3LGE_0kt04Dcs78QqFI5Hpfl6GtFpMTlfCSXq7h8', // Thay thế bằng API key của bạn
        },
      });
  ...
  return (
    <div>
      ...
      <h2>
        <img src={item.item.imageUrl} alt={item.item.name} style={{ width: '200px', height: 'auto' }} />
      </h2>
      ...
      />
      <button onClick={handleBuy} className="btn btn-primary container">Mua</button>
